...
- `config_schema.py` holds `CONFIG_SCHEMA`. It validates the `gree_custom:` block, normalizes the MAC addresses and fills the defaults, so an imported item already has the shape above. `__init__.py` imports `CONFIG_SCHEMA` so Home Assistant and hassfest find it.
