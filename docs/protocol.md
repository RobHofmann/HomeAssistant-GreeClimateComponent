...
- The first probe asks for `len(GreeProp)` columns. If it passes there is no limit worth keeping and `max_props` stays `None`. If it fails, a binary search between `MIN_PACK_PROPS` and the first failing size minus one looks for the largest size that still passes. If even `MIN_PACK_PROPS` fails, the limit is set to `MIN_PACK_PROPS` and one warning is logged; the device is most likely answering nothing at all, which the empty status guard in `GreeDevice` then catches.
