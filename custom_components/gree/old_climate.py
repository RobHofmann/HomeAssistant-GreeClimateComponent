...
async def create_gree_device(hass, config):
    ...
    mac_addr = str(config.get(CONF_MAC)).encode().replace(b":", b"")
    timeout = config.get(CONF_TIMEOUT, DEFAULT_TIMEOUT)

    chm = config.get(CONF_HVAC_MODES)
    hvac_modes = [
        getattr(HVACMode, mode.upper())
        for mode in (chm if chm is not None else DEFAULT_HVAC_MODES)
    ]
    ...
    swing_horizontal_modes = (
        cshm if cshm is not None else DEFAULT_SWING_HORIZONTAL_MODES
    )
...
class GreeClimate(ClimateEntity):
    ...
    def __init__(
        ...
        self._port = port
        mac_addr_str: str = mac_addr.decode("utf-8").lower()
        if "@" in mac_addr_str:
    ...
    def GreeGetValues(self, propertyNames):
        ...
        if self.encryption_version == 1:
            ...
            jsonPayloadToSend = (
                '{"cid":"app","i":0,"pack":"'
                + base64.b64encode(
                    cipher.encrypt(Pad(plaintext).encode("utf8"))
                ).decode("utf-8")
                + '","t":"pack","tcid":"'
                + str(self._mac_addr)
    ...
    def SendStateToAc(self, timeout):
        ...
        _LOGGER.debug(
            f"Sending with Buzzer_ON_OFF={buzzer_command_value} (Beeper is {'ENABLED' if self._beeper_enabled else 'DISABLED'})"
        )
        ...
        if self.encryption_version == 1:
            ...
            sentJsonPayload = (
                '{"cid":"app","i":0,"pack":"'
                + base64.b64encode(
                    cipher.encrypt(Pad(statePackJson).encode("utf8"))
                ).decode("utf-8")
                + '","t":"pack","tcid":"'
                + str(self._mac_addr)
                + '","uid":{}'.format(self._uid)
            ...
            cipher = GetGCMCipher(self._encryption_key)
        receivedJsonPayload = FetchResult(
            cipher,
            self._ip_addr,
            self._port,
            timeout,
    ...
    def UpdateHATargetTemperature(self):
        ...
        if self._acOptions["StHt"] and (int(self._acOptions["StHt"]) == 1):
            ...
            temp_c = decode_temp_c(
                SetTem=self._acOptions["SetTem"], TemRec=self._acOptions["TemRec"]
            )  # takes care of 1/2 degrees
            temp_f = gree_c_to_f(
                SetTem=self._acOptions["SetTem"], TemRec=self._acOptions["TemRec"]
            )
            ...
            if self._unit_of_measurement == "°C":
                ...
                _LOGGER.error(
                    "Unknown unit of measurement: %s" % self._unit_of_measurement
                )

            self._target_temperature = display_temp

            _LOGGER.info(
                f"UpdateHATargetTemperature: HA target temp set to: {self._target_temperature} {self._unit_of_measurement}. Device commands: SetTem: {self._acOptions['SetTem']}, TemRec: {self._acOptions['TemRec']}"
            )
        ...
        if self._acOptions["Pow"] == 0:
            ...
            for key, value in MODES_MAPPING.get("Mod").items():
                if value == (self._acOptions["Mod"]):
                    self._hvac_mode = key
        _LOGGER.debug(
            "HA operation mode set according to HVAC state to: " + str(self._hvac_mode)
        )
    ...
    def UpdateHACurrentSwingMode(self):
        ...
        _LOGGER.debug(
            "HA swing mode set according to HVAC state to: " + str(self._swing_mode)
        )
    ...
    def UpdateHACurrentSwingHorizontalMode(self):
        ...
        _LOGGER.debug(
            "HA horizontal swing mode set according to HVAC state to: "
            + str(self._swing_horizontal_mode)
        )
        ...
        if int(self._acOptions["Tur"]) == 1:
            ...
            for key, value in MODES_MAPPING.get("WdSpd").items():
                if value == (self._acOptions["WdSpd"]):
                    self._fan_mode = key
        _LOGGER.debug(
            "HA fan mode set according to HVAC state to: " + str(self._fan_mode)
        )
    ...
    def UpdateHACurrentTemperature(self):
        ...
        if self._external_temperature_sensor:
            ...
            external_sensor_state = self.hass.states.get(
                self._external_temperature_sensor
            )
            if external_sensor_state and external_sensor_state.state not in (
                "unknown",
                "unavailable",
            ):
                try:
                    unit = external_sensor_state.attributes.get(
                        ATTR_UNIT_OF_MEASUREMENT
                    )
                    _LOGGER.debug(
                        f"Using external temperature sensor: {self._external_temperature_sensor}, value: {external_sensor_state.state}, unit: {unit}"
                    )
                    self._current_temperature = self.hass.config.units.temperature(
                        float(external_sensor_state.state), unit
                    )
                    _LOGGER.debug(
                        f"External temperature: {self._current_temperature} {self._unit_of_measurement}"
                    )
                    return
        ...
        if self._has_temp_sensor:
            _LOGGER.debug(
                "method UpdateHACurrentTemperature: TemSen: "
                + str(self._acOptions["TemSen"])
            )
            ...
            if self._temp_sensor_offset is None:  # user hasn't chosen an offset
                ...
                _LOGGER.debug(
                    "method UpdateHACurrentTemperature: User has not chosen an offset, using process_temp_sensor() to automatically determine offset."
                )
            else:
                # User set
                ...
                _LOGGER.debug(
                    f"method UpdateHACurrentTemperature: User has chosen an offset ({self._temp_sensor_offset})"
                )

            temp_f = gree_c_to_f(
                SetTem=temp_c, TemRec=0
            )  # Convert to Fahrenheit using TemRec bit
            ...
            _LOGGER.debug(
                "method UpdateHACurrentTemperature: HA current temperature set with device built-in temperature sensor state : "
    ...
    def UpdateHAOutsideTemperature(self):
        ...
        if self._has_outside_temp_sensor:
            _LOGGER.debug(
                "method UpdateHAOutsideTemperature: OutEnvTem: "
                + str(self._acOptions["OutEnvTem"])
            )
            ...
            if self._temp_sensor_offset is None:  # user hasn't chosen an offset
                ...
                _LOGGER.debug(
                    "method UpdateHAOutsideTemperature: User has not chosen an offset, using process_temp_sensor() to automatically determine offset."
                )
            else:
                # User set
                if self._temp_sensor_offset is True:
                ...
                _LOGGER.debug(
                    f"method UpdateHAOutsideTemperature: User has chosen an offset ({self._temp_sensor_offset})"
                )
            ...
            if self._unit_of_measurement == "°C":
                ...
                _LOGGER.error(
                    "Unknown unit of measurement for outside temperature: %s"
                    % self._unit_of_measurement
                )

            _LOGGER.debug(
                "method UpdateHAOutsideTemperature: HA outside temperature set with device built-in outside temperature sensor state : "
                + str(self._current_outside_temperature)
                + str(self._unit_of_measurement)
            )
    ...
    def UpdateHARoomHumidity(self):
        ...
        if self._has_room_humidity_sensor:
            _LOGGER.debug(
                "method UpdateHARoomHumidity: DwatSen: "
                + str(self._acOptions["DwatSen"])
            )
            self._current_room_humidity = self._acOptions["DwatSen"]
            _LOGGER.debug(
                "method UpdateHARoomHumidity: HA room humidity set with device built-in room humidity sensor state : "
                + str(self._current_room_humidity)
                + "%"
            )
    ...
    def SyncState(self, acOptions={}):
        ...
        if self._has_light_sensor is None:
            ...
            try:
                ...
                _LOGGER.debug(
                    "Could not determine whether device has a built-in light sensor. Retrying at next update()"
                )
            else:
                if light_sensor:
        ...
        if self._has_outside_temp_sensor is None:
            _LOGGER.debug(
                "Attempt to check whether device has an outside temperature sensor"
            )
            try:
                outside_temp_sensor = self.GreeGetValues(["OutEnvTem"])
            except Exception:
                _LOGGER.debug(
                    "Could not determine whether device has an outside temperature sensor. Retrying at next update()"
                )
            else:
        ...
        if self._has_room_humidity_sensor is None:
            ...
            try:
                ...
                _LOGGER.debug(
                    "Could not determine whether device has a room humidity sensor. Retrying at next update()"
                )
            else:
                if humidity_sensor:
    ...
    def update(self):
        ...
        if not self._encryption_key:
            if self.encryption_version == 1:
                key = GetDeviceKey(
                    self._mac_addr, self._ip_addr, self._port, self._timeout
                )
                if key:
    ...
    def target_temperature_step(self):
        _LOGGER.debug(
            "target_temperature_step(): " + str(self._target_temperature_step)
        )
    ...
    def swing_horizontal_mode(self):
        if self._swing_horizontal_modes:
            _LOGGER.debug(
                "swing_horizontal_mode(): " + str(self._swing_horizontal_mode)
            )
    ...
    def outside_temperature(self):
        ...
        if self._has_outside_temp_sensor:
            _LOGGER.debug(
                "outside_temperature(): " + str(self._current_outside_temperature)
            )
        ...
        if s is not None:
            ...
            if not (self._acOptions["Pow"] == 0):
                ...
                if self._unit_of_measurement == "°C":
                    ...
                    _LOGGER.error(
                        "Unable to set temperature. Units not set to °C or °F"
                    )
                    return

                self.SyncState({"SetTem": int(SetTem), "TemRec": int(TemRec)})
                _LOGGER.debug(
                    "method set_temperature: Set Temp to "
                    + str(s)
                    + str(self._unit_of_measurement)
                    + " ->  SyncState with SetTem="
                    + str(SetTem)
                    + ", SyncState with TemRec="
                    + str(TemRec)
                )
    ...
    def set_swing_horizontal_mode(self, swing_horizontal_mode):
        if not (self._acOptions["Pow"] == 0):
            # do nothing if HVAC is switched off
            try:
                swing_lf_rig = MODES_MAPPING.get("SwingLfRig").get(
