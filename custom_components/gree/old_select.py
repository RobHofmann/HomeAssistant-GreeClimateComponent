...
@dataclass
class GreeSelectEntityDescription(OldGreeEntityDescription, SelectEntityDescription):
...
SELECTS: tuple[GreeSelectEntityDescription, ...] = (
    GreeSelectEntityDescription(
        ...
        set_fn=lambda device, value: setattr(
...
async def async_setup_entry(
    ...
    async_add_entities(
        GreeSelectEntity(hass, entry, description) for description in SELECTS
    )


class GreeSelectEntity(OldGreeEntity, SelectEntity, RestoreEntity):
    """Defines a Gree select entity."""

    entity_description: GreeSelectEntityDescription

    def __init__(
        self, hass: HomeAssistant, entry, description: GreeSelectEntityDescription
    ) -> None:
        ...
        if self.entity_description.restore_state:
            ...
            if restored and restored.state not in ("unknown", "unavailable"):
                ...
                if restored.state in self._attr_options:
                    ...
                    _LOGGER.debug(
                        "Restored %s state: %s", self.entity_id, restored.state
                    )
                else:
                    _LOGGER.warning(
                        "Restored state %s not in current options, resetting to None",
                        restored.state,
                    )
                    if self.entity_description.set_fn:
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        if self.entity_description.set_fn:
            ...
            _LOGGER.info(
                "Selected %s: %s", self.entity_description.property_key, option
            )
    ...
    async def async_update(self) -> None:
        ...
        if self.entity_description.options_fn:
            ...
            if new_options != self._attr_options:
                ...
                _LOGGER.debug(
                    "Updated temperature sensor options: %s", self._attr_options
                )
