...
@dataclass
class GreeSwitchEntityDescription(OldGreeEntityDescription, SwitchEntityDescription):
...
SWITCHES: tuple[GreeSwitchEntityDescription, ...] = (
    ...
    GreeSwitchEntityDescription(
        property_key="sleep",
        icon="mdi:sleep",
        value_fn=lambda device: device._acOptions.get("SwhSlp") == 1
        and device._acOptions.get("SlpMod") == 1,
        set_fn=lambda device, value: device.SyncState(
            {"SwhSlp": 1 if value else 0, "SlpMod": 1 if value else 0}
        ),
    ...
    GreeSwitchEntityDescription(
        ...
        set_fn=lambda device, value: device.SyncState(
            {"AntiDirectBlow": 1 if value else 0}
        ),
    ...
    GreeSwitchEntityDescription(
        ...
        value_fn=lambda device: device._acOptions.get("LigSen")
        == 0,  # LigSen=0 means sensor is active
        set_fn=lambda device, value: device.SyncState(
            {"Lig": 1, "LigSen": 0} if value else {"LigSen": 1}
        ),
...
async def async_setup_entry(
    ...
    async_add_entities(
        GreeSwitchEntity(hass, entry, description) for description in SWITCHES
    )


class GreeSwitchEntity(OldGreeEntity, SwitchEntity, RestoreEntity):
